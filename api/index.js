...
export async function GET(request) {
	...
	// javplayer special handler (scraping di resolver karena IP Heroku diblokir Cloudflare)
	if (urlData.includes("javplayer.cc") || urlData.includes("123av.com")) {
		try {
			let epUrl = urlData;
			let title = null;

			if (urlData.includes("123av.com")) {
				const page = await fetch(urlData, {
					headers: {
						"User-Agent":
				...
				if (!page.ok) {
					return Response.json({
						message: `123av mengembalikan status ${page.status}`,
						status: page.status,
					});
				}

				const html = await page.text();
				const m = html.match(/JSON\.parse\(\\?'(\[.*?\])\\?'\)/s);
				if (!m) {
					return Response.json({
				...
				let frag = m[1].replace(/\\\\/g, "\\").replace(/\\\//g, "/");
				frag = frag.replace(
					/\\u([0-9a-fA-F]{4})/g,
					(_, x) => String.fromCharCode(Number.parseInt(x, 16)),
				);
				const episodes = JSON.parse(frag);
				if (!episodes.length) {
					return Response.json({
						message: "Tidak ada episode.",
						status: 404,
					});
				}

				epUrl = episodes[0].url;
				const t = html.match(/<title>(.*?)<\/title>/s);
				if (t) {
					title = t[1].trim().split(" — ")[0].trim();
				}
			}

			const hashMatch = epUrl.match(/\/e\/([a-z0-9_]+)/i);
			if (!hashMatch) {
				return Response.json({
					message: "ID javplayer tidak ditemukan.",
					status: 404,
				});
			}

			const api = await fetch(
				`https://javplayer.cc/stream?id=${hashMatch[1]}`,
				{
					headers: {
						"User-Agent":
							"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36",
			...
			if (!api.ok) {
				return Response.json({
					message: `API stream status ${api.status}`,
					status: api.status,
				});
			}

			const data = await api.json();
			if (data.status !== "ok" || !data.media?.stream) {
				return Response.json({
